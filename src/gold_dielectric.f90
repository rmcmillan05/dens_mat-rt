...
!PROGRAM gold_dielectric
!    USE double
!    IMPLICIT NONE
!    REAL(KIND=DP) :: lambda
!    COMPLEX(KIND=DP) :: e_au
!    REAL(KIND=DP) :: a = 200.0_DP
!    REAL(KIND=DP) :: b = 1000.0_DP
!    INTEGER :: n = 2000
!    INTEGER :: i
!
!    DO i = 1,2000
!        lambda = (i-1)*(b-a)/(n-1) + a
!        e_au = au_di_function(lambda)
!        WRITE(*,'(3ES16.5)') lambda, REAL(e_au,KIND=DP), IMAG(e_au)
!    ENDDO
...
CONTAINS
    ...
    REAL(KIND=DP),INTENT(IN)    :: omega ! Given in a.u.
    REAL(KIND=DP) :: lambda
